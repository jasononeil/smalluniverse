...
function IngredientList(name:String, ingredients:Array<{ingredient:String, ticked:Bool, info:Null<String>}>, ?extraItems:Html<AppAction>) {
	return [
		...
		section([className("IngredientList")], [
			...
			ListView(ingredients.map(i -> {
				final info = i.info;
				return ListItemLink(label([], [
					checkbox([checked(i.ticked), className("IngredientList__Checkbox")]),
					i.ingredient,
					info != null ? span([className("IngredientList__Info")], info) : []
				]), appRouter.uriForIngredientPage({
